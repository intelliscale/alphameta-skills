...
- Collect 3-5 years from `filing <symbol>` (10-K/10-Q)
...
- Use management guidance from latest earnings + `filing <symbol>`
...
Cost of Debt = (Interest Expense ÷ Total Debt) × (1 - Tax Rate)
  - Extract from `filing <symbol>`

WACC = (E/V × Ke) + (D/V × Kd)
  - E = Market Cap (from `calc-index <symbol>` → total_market_value)
  - D = Net Debt (from `filing <symbol>`, Total Debt - Cash)
  - V = E + D
```
...
- Net Debt (Total Debt - Cash, from `filing <symbol>`)
+ Non-operating Assets (if any)
= Equity Value

Price Per Share = Equity Value / Diluted Shares (from `financial-report <symbol> BS`)
```
...
- Market Cap: `calc-index <symbol>` → total_market_value
- EPS (TTM): `consensus <symbol>` → eps_ttm
- P/E: `calc-index <symbol>` → pe
- P/B: `calc-index <symbol>` → pb
- Net Debt: from `filing <symbol>` (Total Debt - Cash)
...
AlphaMeta CLI does NOT provide M&A transaction data. Use web search:
...
Current Price: $XX (from `quote <symbol>`)
Implied Upside: +XX%
```
