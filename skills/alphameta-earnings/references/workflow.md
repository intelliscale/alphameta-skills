...
## ⚠️ CRITICAL: USE ALPHAMETA CLI TO IDENTIFY THE LATEST EARNINGS ⚠️

Training data is OUTDATED. Always use the AlphaMeta CLI to find and verify the latest earnings quarter. Do NOT rely on knowledge cutoff or web search as the primary method.

## Phase 1: Earnings Data Collection

### Step 1: Identify the Latest Earnings Period

**Use the AlphaMeta CLI as the primary method** to detect the latest reported quarter:

Use `query=filing` to discover available options for the filing command. Run `filing <symbol>` to list recent filings and filter for the latest 10-Q (quarterly) or 10-K (annual) by date. Note the filing details, then extract the period end date (e.g., "For the quarterly period ended [DATE]").

**Step 1b: Map period-end date to fiscal quarter**
...
**Step 1c: Verify the filing is recent**
...
- Use `query=news` to discover available options, then `news <symbol>` to search for earnings-related news
- As a last resort, ask the user to specify the quarter

**⚠️ HK/CN symbol format note**: Leading zeros in HK symbols can cause empty results. If `09988.HK` returns no data, try `9988.HK`. Always verify with `query=filing` for the symbol format.

**MANDATORY VERIFICATION before proceeding:**

- [ ] Used `filing <symbol>` to find the latest 10-Q/10-K (NOT web search or training data)
- [ ] Extracted the period-end date from the filing header
...
After confirming the quarter (Step 1), collect materials **primarily via the AlphaMeta CLI**:

**Primary Materials (REQUIRED) — use AlphaMeta CLI first:**

- **Financial statements** — use `query=financial-report` to discover available options, then `financial-report <symbol> IS`, `financial-report <symbol> BS`, and `financial-report <symbol> CF` to get structured income statement, balance sheet, and cash flow data. This is the **primary source for financial figures** (especially for HK/CN stocks where filing PDFs are not machine-readable).
  - Query each statement kind separately (`IS`, `BS`, `CF`) — `ALL` may return empty for some symbols.
...
- **10-Q or 10-K filing** (for US stocks, already identified in Step 1):
  Use `query=filing` to discover available options — contains full financial statements, MD&A, and risk factors. For Q4 use the 10-K (annual); for Q1-Q3 use the 10-Q.
  - **Note**: HK/CN filings are typically PDFs. Use `financial-report <symbol>` instead for structured data.

- **Earnings press release / 8-K** — often filed as 8-K on the same day:
  Use `query=filing` to discover available options, then `filing <symbol>` to list and filter for 8-K filings near the same date as the 10-Q. The 8-K often contains the earnings press release as an exhibit.

- **Earnings-related news** via AlphaMeta:
  Use `query=news` to discover available options, then `news <symbol>` to search for earnings-related news. Look for titles containing "earnings", "results", "revenue", "Q[X]". Read full articles for analyst commentary and consensus data.

- **Earnings call transcript** — NOT available via AlphaMeta, use web search:
...
- **Consensus estimates** — Via `query=consensus` then `consensus <symbol>` and web search (supplement)
...
Use `query=financial-report` to discover available options, then `financial-report <symbol> IS p8` for multi-period margin trends.

### Step 8: Guidance Analysis

If company provided guidance:
- Compare new guidance to prior guidance
- Compare to internal estimates and consensus estimates
...
Use `query=consensus`, `query=calc-index`, `query=kline`, and `query=financial-report` to discover available options and output fields, and fetch the market and financial data needed for valuation.
...
- [ ] All SEC filings hyperlinked (use `filing <symbol>` to find filing details)
